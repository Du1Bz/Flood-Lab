"""
pages/report.py
---------------
定型レポートビュー（週次スナップショット）。

期間を選ぶと固定レイアウトで以下を表示:
- KPM推移グラフ
- 最も活躍したマップ TOP3
- 最高/最低KDA試合
- 区分別勝率
- 前週比変化（KDA / 命中率 / 勝率）
- インサイト（Lv.2統計ベース）
"""

from __future__ import annotations

from datetime import date, timedelta

import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
import streamlit as st

from src.logic.insights import run_insights
from src.utils.display import PLAYLIST_DISPLAY, RESULT_DISPLAY
from src.utils.helpers import format_duration, week_label, month_label

st.set_page_config(page_title="定型レポート | Flood-Lab", layout="wide")

from src.utils.session import load_data, sidebar_refresh
sidebar_refresh()
df, _ = load_data()

if df is None or df.empty:
    st.warning("データをロードできませんでした。")
    st.stop()

df_all  = df.copy()
df_stat = df_all[df_all["exclude_flag"] == ""].copy()

# ==================================================
# サイドバー: 期間選択
# ==================================================

with st.sidebar:
    st.header("期間選択")

    period_mode = st.radio("単位", ["週", "月", "セッション", "カスタム"], index=0)

    if period_mode == "週":
        today = pd.Timestamp.now(tz="Asia/Tokyo")
        weeks = []
        for i in range(26):
            monday = today - timedelta(days=today.weekday() + i * 7)
            monday = monday.replace(hour=0, minute=0, second=0, microsecond=0)
            sunday = monday + timedelta(days=6, hours=23, minutes=59, seconds=59)
            label  = week_label(monday)
            weeks.append((label, monday, sunday))

        week_labels = [w[0] for w in weeks]
        sel_label   = st.selectbox("週を選択", week_labels, index=0)
        sel_week    = next(w for w in weeks if w[0] == sel_label)
        period_from, period_to = sel_week[1], sel_week[2]
        period_label = sel_label

        prev_from = period_from - timedelta(weeks=1)
        prev_to   = period_from - timedelta(seconds=1)

    elif period_mode == "月":
        today = pd.Timestamp.now(tz="Asia/Tokyo")
        months = []
        for i in range(12):
            m = today.month - i
            y = today.year
            while m <= 0:
                m += 12
                y -= 1
            first = pd.Timestamp(year=y, month=m, day=1, tz="Asia/Tokyo")
            if m == 12:
                last = pd.Timestamp(year=y + 1, month=1, day=1, tz="Asia/Tokyo") - pd.Timedelta(seconds=1)
            else:
                last = pd.Timestamp(year=y, month=m + 1, day=1, tz="Asia/Tokyo") - pd.Timedelta(seconds=1)
            months.append((month_label(first), first, last))

        month_labels = [m[0] for m in months]
        sel_label    = st.selectbox("月を選択", month_labels, index=0)
        sel_month    = next(m for m in months if m[0] == sel_label)
        period_from, period_to = sel_month[1], sel_month[2]
        period_label = sel_label

        prev_from = period_from - pd.DateOffset(months=1)
        prev_to   = period_from - pd.Timedelta(seconds=1)

    elif period_mode == "セッション":
        # セッションごとの開始・終了時刻を計算
        JST = "Asia/Tokyo"
        if "session_id" in df_stat.columns:
            sess_info = (
                df_stat.groupby("session_id")
                .agg(
                    start=("played_at", "min"),
                    end=("played_at", "max"),
                    games=("result_flag", "count"),
                )
                .reset_index()
                .sort_values("start", ascending=False)
            )
            # セッション名: "2026.5.17 20:00 ~"
            def fmt_session(t):
                t_jst = t.tz_convert(JST)
                return f"{t_jst.year}.{t_jst.month}.{t_jst.day} {t_jst.hour:02d}:{t_jst.minute:02d} ~"
            sess_info["label"] = sess_info["start"].apply(fmt_session)

            sess_labels = sess_info["label"].tolist()
            sel_label   = st.selectbox("セッションを選択", sess_labels, index=0)
            sel_row     = sess_info[sess_info["label"] == sel_label].iloc[0]

            period_from  = sel_row["start"] - pd.Timedelta(seconds=1)
            period_to    = sel_row["end"]   + pd.Timedelta(hours=2)
            period_label = sel_label.replace(" ~", "")

            # 前セッションを前期間として使う
            cur_sid  = sel_row["session_id"]
            prev_sid = cur_sid - 1
            prev_rows = sess_info[sess_info["session_id"] == prev_sid]
            if not prev_rows.empty:
                pr = prev_rows.iloc[0]
                prev_from = pr["start"] - pd.Timedelta(seconds=1)
                prev_to   = pr["end"]   + pd.Timedelta(hours=2)
            else:
                prev_from = prev_to = period_from
        else:
            st.warning("セッションデータがありません。")
            period_from = period_to = pd.Timestamp.now(tz="Asia/Tokyo")
            prev_from = prev_to = period_from
            period_label = "セッション"

    else:  # カスタム
        col_a, col_b = st.columns(2)
        date_from = col_a.date_input("開始日", value=date.today() - timedelta(days=7))
        date_to   = col_b.date_input("終了日", value=date.today())
        period_from = pd.Timestamp(date_from, tz="Asia/Tokyo")
        period_to   = pd.Timestamp(date_to, tz="Asia/Tokyo").replace(hour=23, minute=59, second=59)
        period_label = f"{date_from} 〜 {date_to}"

        delta = period_to - period_from
        prev_from = period_from - delta
        prev_to   = period_from - pd.Timedelta(seconds=1)

# ==================================================
# 対象期間のDataFrame
# ==================================================

cur_df  = df_stat[(df_stat["played_at"] >= period_from) & (df_stat["played_at"] <= period_to)]
prev_df = df_stat[(df_stat["played_at"] >= prev_from)   & (df_stat["played_at"] <= prev_to)]

# ==================================================
# メインコンテンツ
# ==================================================

st.title(f"📊 定型レポート — {period_label}")
st.caption(f"{period_from.strftime('%Y/%m/%d')} 〜 {period_to.strftime('%Y/%m/%d')}")

if cur_df.empty:
    st.info("この期間に試合データがありません。")
    st.stop()

# ==================================================
# サマリーカード
# ==================================================

col1, col2, col3, col4, col5 = st.columns(5)
col1.metric("試合数",       len(cur_df))
col2.metric("勝率",         f"{cur_df['result_flag'].mean():.1%}")
col3.metric("平均K/D",      f"{cur_df['kd_ratio'].mean():.2f}")
acc = cur_df["accuracy"].dropna()
col4.metric("平均命中率",   f"{acc.mean():.1%}" if len(acc) else "—")
col5.metric("CSR増減合計",  f"{cur_df['csr_delta'].sum():+.0f}" if cur_df["csr_delta"].notna().any() else "—")

st.divider()

# ==================================================
# eMMR v2 推移（x軸: 試合番号）
# ==================================================

st.subheader("📈 eMMR v2 推移")

emmr_df = cur_df[
    ~cur_df["playlist"].isin(["custom", "casual", "btb", "pve", "minigame"])
].dropna(subset=["emmr_v2"]).sort_values("played_at").copy()
emmr_df["x"] = range(len(emmr_df))

if not emmr_df.empty:
    fig = go.Figure()
    for playlist, grp in emmr_df.groupby("playlist"):
        color_map = {
            "ranked_arena":   "#636EFA",
            "ranked_slayer":  "#EF553B",
            "ranked_doubles": "#00CC96",
            "ranked_ffa":     "#FECB52",
            "ranked_snipers": "#B6E880",
        }
        fig.add_trace(go.Scatter(
            x=grp["x"], y=grp["emmr_v2"],
            mode="lines+markers",
            name=PLAYLIST_DISPLAY.get(playlist, playlist),
            line=dict(color=color_map.get(playlist, "#aaa"), width=2),
            marker=dict(size=5),
        ))
    fig.update_layout(
        xaxis_title="試合番号", yaxis_title="eMMR v2",
        height=600, margin=dict(t=10, b=10),
        legend=dict(orientation="h", y=1.05),
    )
    st.plotly_chart(fig, width="stretch")
else:
    st.info("eMMR v2 のデータがありません。")

st.divider()

# ==================================================
# マップ × ルール 勝率ヒートマップ
# ==================================================

st.subheader("🗺️ マップ × ルール 勝率")

hm_df = cur_df.groupby(["map_name", "rule_name"]).agg(
    勝率=("result_flag", "mean"),
    試合数=("result_flag", "count"),
).reset_index()

if not hm_df.empty:
    pivot = hm_df.pivot(index="map_name", columns="rule_name", values="勝率")
    pivot_n = hm_df.pivot(index="map_name", columns="rule_name", values="試合数").fillna(0).astype(int)

    # NaN（空白セル）を -1 に置換してグレー表示用に使う
    import numpy as np
    z_vals = pivot.values.copy().astype(float)
    text_vals = []
    for r in pivot.index:
        row_text = []
        for c in pivot.columns:
            v = pivot.loc[r, c]
            n = pivot_n.loc[r, c] if c in pivot_n.columns and r in pivot_n.index else 0
            if pd.isna(v):
                row_text.append("—")
            else:
                row_text.append(f"{v:.0%} ({n}試合)")
        text_vals.append(row_text)

    # NaN を -1 にしてカラースケールでグレーとして描画
    z_display = np.where(np.isnan(z_vals), -1, z_vals)

    colorscale = [
        [0.000, "#2a2a2a"],   # z=-1 → グレー（空白セル）
        [0.499, "#2a2a2a"],   # z=-1 の範囲の終わり
        [0.500, "#7f1d1d"],   # z=0  → 深赤（0%）
        [0.675, "#dc2626"],   # z=0.35
        [0.740, "#f97316"],   # z=0.48
        [0.750, "#eab308"],   # z=0.50 → 黄（50%）
        [0.760, "#84cc16"],   # z=0.52
        [0.825, "#16a34a"],   # z=0.65
        [1.000, "#14532d"],   # z=1.0  → 深緑（100%）
    ]

    fig = go.Figure(data=go.Heatmap(
        z=z_display,
        x=pivot.columns.tolist(),
        y=pivot.index.tolist(),
        colorscale=colorscale,
        zmin=-1, zmax=1,
        text=text_vals,
        texttemplate="%{text}",
        textfont=dict(size=11),
        hovertemplate="マップ: %{y}<br>ルール: %{x}<br>%{text}<extra></extra>",
        showscale=True,
        colorbar=dict(
            tickvals=[0, 0.25, 0.5, 0.75, 1.0],
            ticktext=["0%", "25%", "50%", "75%", "100%"],
            thickness=12,
            len=0.8,
        ),
    ))
    fig.update_layout(
        xaxis_title="ルール",
        yaxis_title="マップ",
        height=max(300, len(pivot) * 32 + 100),
        margin=dict(t=20, b=20, l=120, r=80),
    )
    st.plotly_chart(fig, width="stretch")
else:
    st.info("データがありません。")

st.divider()

# ==================================================
# 最高/最低KDA試合
# ==================================================

st.subheader("🎯 注目試合")

col_best, col_worst = st.columns(2)

best  = cur_df.loc[cur_df["kda"].idxmax()]  if cur_df["kda"].notna().any() else None
worst = cur_df.loc[cur_df["kda"].idxmin()]  if cur_df["kda"].notna().any() else None

def _match_card(row: pd.Series, label: str):
    st.markdown(f"**{label}**")
    st.write(f"📅 {pd.Timestamp(row['played_at']).tz_convert('Asia/Tokyo').strftime('%m/%d %H:%M')}")
    st.write(f"🗺️ {row.get('map_name', '—')} / {row.get('rule_name', '—')}")
    st.write(f"🏆 {RESULT_DISPLAY.get(row.get('result', ''), '—')}")
    st.write(f"KDA: **{row.get('kda', '—'):.2f}**")
    st.write(f"K/D: {row.get('kd_ratio', '—'):.2f}")
    dd = row.get("damage_diff")
    st.write(f"ダメージ差: {int(dd):+,}" if pd.notna(dd) else "ダメージ差: —")

with col_best:
    if best is not None:
        _match_card(best, "✨ 最高KDA試合")

with col_worst:
    if worst is not None:
        _match_card(worst, "💀 最低KDA試合")

st.divider()

# ==================================================
# 区分別勝率
# ==================================================

st.subheader("🏆 区分別勝率")

section_agg = (
    cur_df.groupby("playlist")
    .agg(
        試合数=("result_flag", "count"),
        勝率=("result_flag", "mean"),
        KD=("kd_ratio", "mean"),
        CSR増減=("csr_delta", "sum"),
    )
    .reset_index()
)
section_agg["区分"] = section_agg["playlist"].map(PLAYLIST_DISPLAY)
section_agg["勝率表示"] = section_agg["勝率"].apply(lambda x: f"{x:.1%}")
section_agg["KD表示"]   = section_agg["KD"].apply(lambda x: f"{x:.2f}")
section_agg["CSR"]      = section_agg["CSR増減"].apply(
    lambda x: f"{x:+.0f}" if pd.notna(x) else "—"
)

st.dataframe(
    section_agg[["区分", "試合数", "勝率表示", "KD表示", "CSR"]].rename(columns={
        "勝率表示": "勝率", "KD表示": "K/D", "CSR": "CSR増減合計"
    }),
    hide_index=True,
    width="stretch",
)

st.divider()

# ==================================================
# 前週比変化
# ==================================================

st.subheader("📈 前期間との比較")

def _delta_metric(label: str, cur_val, prev_val, fmt: str = ".2f"):
    if prev_val is None or pd.isna(prev_val) or cur_val is None or pd.isna(cur_val):
        st.metric(label, f"{cur_val:{fmt}}" if cur_val is not None and pd.notna(cur_val) else "—")
        return
    delta = cur_val - prev_val
    st.metric(label, f"{cur_val:{fmt}}", delta=f"{delta:+{fmt}}")

col1, col2, col3, col4 = st.columns(4)

with col1:
    cur_wr  = cur_df["result_flag"].mean() * 100 if len(cur_df) else None
    prev_wr = prev_df["result_flag"].mean() * 100 if len(prev_df) else None
    _delta_metric("勝率(%)", cur_wr, prev_wr, fmt=".1f")

with col2:
    cur_kda  = cur_df["kda"].mean() if len(cur_df) else None
    prev_kda = prev_df["kda"].mean() if len(prev_df) else None
    _delta_metric("平均KDA", cur_kda, prev_kda)

with col3:
    cur_acc  = cur_df["accuracy"].dropna().mean() * 100 if cur_df["accuracy"].notna().any() else None
    prev_acc = prev_df["accuracy"].dropna().mean() * 100 if len(prev_df) and prev_df["accuracy"].notna().any() else None
    _delta_metric("命中率(%)", cur_acc, prev_acc, fmt=".1f")

with col4:
    cur_kpm  = cur_df["kpm"].dropna().mean() if cur_df["kpm"].notna().any() else None
    prev_kpm = prev_df["kpm"].dropna().mean() if len(prev_df) and prev_df["kpm"].notna().any() else None
    _delta_metric("平均KPM", cur_kpm, prev_kpm, fmt=".3f")

if prev_df.empty:
    st.caption("前期間のデータがないため比較できません。")

st.divider()

# ==================================================
# 🎯 プレイヤー仮説検証セクション: 最速撃ち vs 丁寧なショット
# ==================================================
st.divider()
st.subheader("🎯 プレイスタイル仮説検証: 『丁寧なショットは正義か？』")
st.caption("「最速撃ちでエイムがブレてデス量産」を防げているかをTrueSkill2の期待値から検証します。")

# 列追加用にコピーを作る（SettingWithCopyWarning回避）
hyp_df = cur_df.copy()
hyp_df["result_str"]   = hyp_df["result_flag"].map({1: "Win", 0: "Loss"})
hyp_df["accuracy_pct"] = hyp_df["accuracy"] * 100

hyp_c1, hyp_c2 = st.columns(2)

with hyp_c1:
    st.write("📈 **命中率と生存能力(D-RPI)の相関**")

    # 散布図にトレンドライン（傾向線）を乗せる
    fig_hyp1 = px.scatter(
        hyp_df,
        x="accuracy_pct",
        y="d_rpi",
        color="result_str",
        color_discrete_map={"Win": "#1D9E75", "Loss": "#D85A30"},
        labels={"accuracy_pct": "命中率 (%)", "d_rpi": "生存能力 (D-RPI / 高いほど優秀)"},
        hover_data=["map_name", "kills", "deaths"],
        trendline="ols",  # 全体の傾向線を自動計算（要: statsmodels）
    )

    # Y=1.0（AIの想定通りのデス数）に基準線を引く
    fig_hyp1.add_hline(y=1.0, line_dash="dash", line_color="#888888", annotation_text="AIの予想デス")
    fig_hyp1.update_layout(
        plot_bgcolor="rgba(0,0,0,0)",
        xaxis=dict(showgrid=True, gridcolor="rgba(0,0,0,0.1)"),
        yaxis=dict(showgrid=True, gridcolor="rgba(0,0,0,0.1)"),
    )
    st.plotly_chart(fig_hyp1, width="stretch")

    st.info(
        "💡 **見方:** 傾向線が右肩上がりなら『命中率が上がるとデス数が抑えられる』説が立証されます。 "
        "Winの点が右上のエリア（高命中率・高生存）に集まっているかチェック！"
    )

with hyp_c2:
    st.write("📊 **勝敗による命中率の分布の差**")

    # 箱ひげ図で勝敗ごとの命中率のパーセンタイルを可視化
    fig_hyp2 = px.box(
        hyp_df,
        x="result_str",
        y="accuracy_pct",
        color="result_str",
        color_discrete_map={"Win": "#1D9E75", "Loss": "#D85A30"},
        labels={"result_str": "試合結果", "accuracy_pct": "命中率 (%)"},
        points="all",  # 全データをドットとしても横に表示
    )
    fig_hyp2.update_layout(showlegend=False)
    st.plotly_chart(fig_hyp2, width="stretch")

    # 簡単な統計計算をしてテキストでフィードバック
    win_acc_med  = hyp_df[hyp_df["result_flag"] == 1]["accuracy_pct"].median()
    loss_acc_med = hyp_df[hyp_df["result_flag"] == 0]["accuracy_pct"].median()

    if not pd.isna(win_acc_med) and not pd.isna(loss_acc_med):
        diff = win_acc_med - loss_acc_med
        if diff > 0:
            st.success(
                f"🔥 **データは正直:** 勝利時の命中率中央値は **{win_acc_med:.1f}%** で、"
                f"敗北時より **{diff:.1f}% 高い** です！丁寧なショットは確実に効いています。"
            )
        else:
            st.warning(
                f"🤔 **おや？:** 勝利時（{win_acc_med:.1f}%）と敗北時（{loss_acc_med:.1f}%）で"
                "命中率に大きな差がありません。デス増の原因は立ち回りや孤立など別の場所にあるかも？"
            )

# ==================================================
# インサイト
# ==================================================

st.subheader("💡 インサイト")

with st.spinner("インサイトを分析中..."):
    # 期間内のデータでインサイトを計算
    # ただし統計的有意性のためにサンプルが少なければ全期間も参照
    target_df = cur_df if len(cur_df) >= 20 else df_stat
    insights  = run_insights(target_df)

if not insights:
    st.success("この期間に特記すべきインサイトはありません。")
else:
    level_icons = {"info": "ℹ️", "warning": "⚠️", "alert": "🚨"}
    level_colors = {
        "info":    st.info,
        "warning": st.warning,
        "alert":   st.error,
    }

    for ins in insights:
        icon     = level_icons.get(ins["level"], "•")
        renderer = level_colors.get(ins["level"], st.info)
        renderer(f"**{icon} {ins['title']}**\n\n{ins['body']}")